...
1.  **严谨性原则 (Principle of Rigor):**
    *   **禁止投机性实现**: 如无明确授权，**禁止**编写任何未被需求驱动的 fallback/静默吞错/多套实现或未经请求的测试用例。避免“静默失败”(error in silence)和不良编程实践(bad smell)。
    *   **禁止凭空创造**: 未经用户同意，严禁新建文件或引入“简化替代实现”。当项目已有可复用实现时，必须优先复用与修改现有代码(code in hand)，避免重复造轮子。
...
4.  **验证与确认 (Verification & Confirmation):**
    ...
    *   **必要时申请测试授权**: 若完成验证需要新增/修改测试用例，必须先向用户说明理由并征得同意，再进行实现。
